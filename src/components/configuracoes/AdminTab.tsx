...
const SectionDivider = () => <div className="h-px bg-border/60 my-6" />;
...
const AdminTab = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("upload-image", {
        body: { category: "avatar", contentType: file.type, dataBase64: base64, filename: file.name || "avatar.png" },
      });
  ...
  const handleAvatarRemove = async () => {
    setUploadingAvatar(true);
    try {
      const { data, error } = await supabase.functions.invoke("upload-image", { body: { category: "avatar", remove: true } });
      if (error || !(data as { ok?: boolean })?.ok) {
  ...
  const handleSave = () => {
    toast({ title: "Dados atualizados", description: "Os dados do administrador foram salvos com sucesso." });
  };
  ...
  const handleChangeEmail = async () => {
    ...
    if (user?.source !== "supabase") {
      toast({ title: "Indisponível no modo demo", description: "Faça login com uma conta real para trocar o email.", variant: "destructive" });
      return;
    }
    setSavingEmail(true);
    ...
    toast({ title: "Confirmação enviada", description: `Enviamos um link de confirmação para ${target}. O email só será alterado após a confirmação.` });
    setEditingEmail(false);
  };
  ...
  const handleChangePassword = async () => {
    ...
    if (user?.source !== "supabase") {
      toast({ title: "Indisponível no modo demo", description: "Faça login com uma conta real para alterar a senha.", variant: "destructive" });
      return;
    }
    setSavingPwd(true);
  ...
  return (
    <SectionShell
      ...
      title="Meu acesso"
      description="Gerencie seus dados pessoais, segurança e conformidade"
      footer={
        <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
          <Button variant="outline" className="rounded-xl px-6 w-full sm:w-auto" onClick={() => { setForm({ nome: user?.nome ?? "", telefone: "" }); setPwd({ nova: "", confirmar: "" }); setEditingEmail(false); setNovoEmail(""); }}>
            Descartar
          </Button>
      ...
      <div className="space-y-1">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-muted/30 rounded-2xl border border-border/40">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-0.5 mb-3">PNG, JPG ou WEBP até 2MB.</p>
            <div className="flex items-center gap-2 flex-wrap">
              <label className={uploadingAvatar ? "cursor-wait opacity-60 pointer-events-none" : "cursor-pointer"}>
                <input type="file" accept="image/png,image/jpeg,image/webp" onChange={handleAvatarUpload} className="hidden" disabled={uploadingAvatar} />
              ...
              {avatarPreview && (
                <Button variant="outline" size="sm" className="rounded-xl text-xs h-8" onClick={handleAvatarRemove} disabled={uploadingAvatar}>
                  Remover
                </Button>
              )}
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.nome} onChange={(e) => updateField("nome", e.target.value)} placeholder="Seu nome" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Telefone</label>
              <input type="text" value={form.telefone} onChange={(e) => updateField("telefone", formatPhone(e.target.value))} placeholder="(99) 9 9999-9999" className={inputClass} />
            </div>
          </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-4 py-3 bg-muted/30 border border-border rounded-xl">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{currentEmail || "—"}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Email atual da conta</p>
                  </div>
                </div>
                <Button variant="outline" size="sm" className="rounded-xl text-xs h-9 shrink-0" onClick={startEditEmail} disabled={!user}>
                  Trocar email
                </Button>
              </div>
            ) : (
              <div className="space-y-3">
                <input type="email" value={novoEmail} onChange={(e) => setNovoEmail(e.target.value)} placeholder="novoemail@exemplo.com" className={inputClass} autoFocus />
                <div className="flex flex-col sm:flex-row gap-2">
                  <Button size="sm" className="rounded-xl text-xs h-9 gap-2" onClick={handleChangeEmail} disabled={savingEmail || !novoEmail.trim()}>
                    {savingEmail ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />}
                    Confirmar alteração
                  </Button>
                  <Button variant="outline" size="sm" className="rounded-xl text-xs h-9 gap-2" onClick={cancelEditEmail} disabled={savingEmail}>
                    <X className="h-3.5 w-3.5" /> Cancelar
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                  Um link de confirmação será enviado para o novo email. A troca só é efetivada após a confirmação.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="password" value={pwd.nova} onChange={(e) => setPwd((p) => ({ ...p, nova: e.target.value }))} placeholder="Mínimo 6 caracteres" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Confirme a nova senha</label>
              <input type="password" value={pwd.confirmar} onChange={(e) => setPwd((p) => ({ ...p, confirmar: e.target.value }))} placeholder="Repita a nova senha" className={inputClass} />
            </div>
          </div>
          <div className="flex justify-end">
            <Button className="rounded-xl px-5 gap-2" onClick={handleChangePassword} disabled={savingPwd || !pwd.nova || !pwd.confirmar}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 bg-muted/30 rounded-2xl border border-border/40">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Gera um PDF resumindo as práticas de proteção de dados aplicadas neste laboratório.</p>
            </div>
            <Button className="rounded-xl px-4 gap-2 shrink-0" disabled={gerandoLGPD} onClick={async () => {
              setGerandoLGPD(true);
              try {
                ...
                toast({ title: "Falha ao gerar relatório", description: err instanceof Error ? err.message : String(err), variant: "destructive" });
              } finally {
