...
export async function renderExameComLayout(
  ...
  const tituloHtml = `<table class="exame-header-band" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;background-color:#f7f8f9 !important;margin:0 0 12px 0;page-break-after:avoid;break-after:avoid;page-break-inside:avoid;break-inside:avoid;-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color-adjust:exact !important;font-family:Helvetica,Arial,sans-serif;line-height:1.6;"><tr style="height:32px;"><td style="padding:10px 10px;vertical-align:middle;text-align:left;font-size:10pt;font-weight:700;color:#000;text-transform:uppercase;letter-spacing:0.2px;">${exameNome}</td><td style="padding:10px 10px;vertical-align:middle;text-align:right;font-size:8pt;font-weight:700;color:#000;white-space:nowrap;">${dataColetaLabel ?? ""}</td></tr></table>`;

  // Força Courier no corpo dos resultados (espelhando o padrão do laudo de referência),
  // mantendo Helvetica no título do exame.
  const html = `<div class="exame-bloco-custom" style="margin-bottom:20px;page-break-inside:avoid;break-inside:avoid;font-family:Courier,'Courier New',monospace;white-space:break-spaces;">${tituloHtml}<div style="font-size:12pt;line-height:1.4;color:#1a1a2e;font-family:Courier,'Courier New',monospace;white-space:break-spaces;">${corpo.replace(/^\s+/, "").replace(/\s+$/, "")}</div></div>`;
  return { html, margins };
}
