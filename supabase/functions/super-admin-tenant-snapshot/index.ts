...
Deno.serve(async (req) => {
  ...
  const [usersRes, usersTotalRes, atendRes, atendTotalRes, payRes, pacRes, adminRes, unidadesRes, examesRes, profilesByRoleRes, billingRes, planLookupRes, registryRes] = await Promise.all([
    ...
    admin.from("tenant_registry").select("runtime_mode, database_strategy, db_provider, db_region, schema_version, last_health_check, last_health_result").eq("tenant_id", tenantId).maybeSingle(),
  ]);
