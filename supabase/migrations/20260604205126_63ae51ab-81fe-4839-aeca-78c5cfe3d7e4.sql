...
BEGIN
  ...
  -- Se justificativa foi fornecida, seta para o trigger de auditoria
  ...
  IF _patch IS NOT NULL AND _patch <> '{}'::jsonb THEN
    UPDATE public.atendimentos SET
      ...
      motivo_cancelamento = CASE WHEN _motivo_cancel IS NOT NULL THEN _motivo_cancel
                                 WHEN _patch ? 'motivo_cancelamento' THEN NULLIF(_patch->>'motivo_cancelamento','')
